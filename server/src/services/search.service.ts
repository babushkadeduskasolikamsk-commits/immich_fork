...
export class SearchService extends BaseService {
  ...
  async searchSmart(auth: AuthDto, dto: SmartSearchDto): Promise<SearchResponseDto> {
    ...
    // --- Translation refinement for text queries ---
    let queryText = dto.query || '';
    let queryLanguage = dto.language;

    if (dto.query) {
      const translationApiUrl = process.env.TRANSLATE_QUERY_API_URL;
      if (translationApiUrl) {
        try {
          ...
          if (response.ok) {
            const { payload } = await response.json() as { payload: { englishQuery: string } };
            if (payload.englishQuery && payload.englishQuery !== dto.query) {
              console.log(`Translated query: "${dto.query}" → "${payload.englishQuery}"`);
              queryText = payload.englishQuery;
              queryLanguage = 'en'; // translated text is English
            }
          } else {
            // Log non-ok response but fallback to original
            const errorText = await response.text().catch(() => '');
            console.log(`Translation service returned ${response.status}: ${errorText}`);
          }
        } catch (error) {
          // Log network or parsing errors, fallback to original query
          console.log('Translation service call failed:', error);
        }
      } else {
        console.log('TRANSLATE_QUERY_API_URL not set – skipping query translation');
      }
    }

    // --- Embedding generation using (possibly translated) query ---
    let embedding;
    if (dto.query) {
      // Use queryText and queryLanguage which may have been updated by translation
      const key = machineLearning.clip.modelName + queryText + queryLanguage;
      embedding = this.embeddingCache.get(key);
      if (!embedding) {
        embedding = await this.machineLearningRepository.encodeText(queryText, {
